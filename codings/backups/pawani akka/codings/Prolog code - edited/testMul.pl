...
% multiplication([X|A],[Y|B],W) :- mulByOneDeleteZero([X|A],Y,R1),
% multiplication([X|A],B,R2), plus1(R11,R22,W),addToListFront(0,R22,R2)
% ,append(R1,[0],R11).

%multiplication([X|A],[B],W) :- mulByOneDeleteZero([X|A],B,W).
% multiplication([X|A],[Y|B],W) :-
% mulByOneDeleteZero([X|A],Y,R1),append(R1,[0],R11), plus1(R11,R2,W) ,
% multiplication([X|A],B,R2).
